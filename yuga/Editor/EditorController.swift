...
class EditorController: UIViewController {
    ...
    fileprivate(set) lazy var saveButton: UIButton = {
    ...
    init(_ storyContent: String, _ storyTitle: String? = nil) {
        self.sampleHTML = storyContent
        self.sampleTitle = storyTitle
        super.init(nibName: nil, bundle: nil)
    }
    ...
    @objc func rightButtonAction(sender: UIBarButtonItem) {
        ...
        storyDict.setValue(getHTML(), forKey: "text")
        storyDict.setValue(titleTextField.text, forKey: "title")
//        storyDict.setValue(storyTags.text!.split(separator: ","), forKey: "tagList")
        func onComplete(_ serverResponse : Int){
            _ = navigationController?.popViewController(animated: true)
        }
        storyModel.createStoryWithTags(storyDict, onComplete)
...
//         Uncomment to add a border
        ...
        if let sampleTitle = sampleTitle {
            titleTextField.text = sampleTitle
        }
    }
    ...
    override var keyCommands: [UIKeyCommand] {
        if richTextView.isFirstResponder {
            return [ UIKeyCommand(input:"B", modifierFlags: .command, action:#selector(toggleBold), discoverabilityTitle:NSLocalizedString("Bold", comment: "Discoverability title for bold formatting keyboard shortcut.")),
                     ...
                     UIKeyCommand(input:"U", modifierFlags: .command, action:#selector(EditorController.toggleUnderline(_:)), discoverabilityTitle: NSLocalizedString("Underline", comment:"Discoverability title for underline formatting keyboard shortcut.")),
...
extension EditorController : Aztec.TextViewFormattingDelegate {
...
extension EditorController : Aztec.FormatBarDelegate {
    ...
    func showLinkDialog(forURL url: URL?, text: String?, range: NSRange, allowTextEdit: Bool = true) {
        ...
        alertController.addTextField(configurationHandler: { [weak self]textField in
            ...
            textField.addTarget(self,
                                action:#selector(EditorController.alertTextFieldDidChange),
...
extension EditorController: TextViewAttachmentDelegate {
...
//
extension EditorController: UIPopoverPresentationControllerDelegate {
...
private extension EditorController
{
    func saveToDisk(image: UIImage) -> URL {
        ...
        return fileURL
//        return itemModel.uploadImageToCloud(image, <#((NSDictionary) -> Void)#>)
    }
    
    func insertImage(_ image: UIImage) {
        
        
        func onSaveToDisk(_ response: NSDictionary){
            let fileURL = URL(string: (response.value(forKey: "data") as! NSDictionary).value(forKey: "mediaLink") as! String)
        ...
        Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(EditorController.timerFireMethod(_:)), userInfo: progress, repeats: true)
        }
        
        itemModel.uploadImageToCloud(image, onSaveToDisk)
    }
...
extension FormattingIdentifier {
    ...
    private func gridicon(_ gridiconType: GridiconType) -> UIImage {
        let size = EditorController.Constants.formatBarIconSize
